...
export const lambdaHandler = async (
  ...
  try {
    ...
    await createSsmParamter(
      getSsmParamName(channelName, EP),
      EP.Password ?? '',
      SsmParameterType.secure
    );
    ...
    const mediaLiveInput = await newMediaLiveInput(
      channelName,
      securityGroup.SecurityGroup?.Id
    );

    const mediaLiveChannel = await newMediaLiveChannel(
      channelName,
      mediaLiveInput,
      EP.Url ?? '',
      EP.Username ?? '',
